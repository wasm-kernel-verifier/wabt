...
class BinaryReader {
 ...
 private:
  ...
  [[nodiscard]] Result ReadComponentSort(ComponentBinarySort* sort,
...
Result BinaryReader::ReadComponentSort(ComponentBinarySort* sort,
                                       ComponentBinaryCoreSort* core_sort) {
  uint8_t sort_code;
  ...
  switch (static_cast<ComponentBinarySort>(sort_code)) {
    case ComponentBinarySort::Core:
      ...
      switch (static_cast<ComponentBinaryCoreSort>(sort_code)) {
        ...
        case ComponentBinaryCoreSort::Instance:
          *sort = ComponentBinarySort::Core;
          *core_sort = static_cast<ComponentBinaryCoreSort>(sort_code);
          break;
        default:
    ...
    case ComponentBinarySort::Instance:
      *sort = static_cast<ComponentBinarySort>(sort_code);
      break;
    default:
...
Result BinaryReader::ReadComponentAlias() {
  ...
  ComponentBinaryCoreSort core_sort;
  CHECK_RESULT(ReadComponentSort(&sort, &core_sort));
  ...
  switch (static_cast<ComponentBinaryAlias>(target_type)) {
    case ComponentBinaryAlias::Outer: {
      if (!(sort == ComponentBinarySort::Core &&
            core_sort == ComponentBinaryCoreSort::Module) &&
          sort != ComponentBinarySort::Component &&
          sort != ComponentBinarySort::Type) {
        ERROR("Invalid sort type: %" PRIx8, static_cast<uint8_t>(sort));
        return Result::Error;
      }
      uint32_t counter, index;
      CHECK_RESULT(ReadU32Leb128(&counter, "counter"));
      CHECK_RESULT(ReadU32Leb128(&index, "index"));
      COMPONENT_CALLBACK(OnAliasOuter, sort, core_sort, counter, index);
      break;
    }
    case ComponentBinaryAlias::Export: {
      ...
      COMPONENT_CALLBACK(OnAliasExport, sort, core_sort, instance_index,
                         export_name);
      break;
    }
    default:
...
Result BinaryReader::ReadComponentType() {
  ...
  while (true) {
    ...
    if (ComponentType::IsPrimitiveType(type_code)) {
      ...
      ComponentBinaryType def_type =
          static_cast<ComponentBinaryType>(type_code);

      switch (def_type) {
        case ComponentBinaryType::Record: {
        ...
        case ComponentBinaryType::Variant: {
          uint32_t case_count;
        ...
        case ComponentBinaryType::List:
        case ComponentBinaryType::ListFixed: {
          ComponentType type;
          CHECK_RESULT(ReadComponentValType(&type));
          if (def_type == ComponentBinaryType::List) {
        ...
        case ComponentBinaryType::Option: {
          ComponentType type;
          CHECK_RESULT(ReadComponentValType(&type));
          COMPONENT_CALLBACK(OnOptionType, type);
          break;
        }
        case ComponentBinaryType::Result: {
        ...
        case ComponentBinaryType::Own: {
          Index index;
          CHECK_RESULT(ReadU32Leb128(&index, "type index"));
          COMPONENT_CALLBACK(OnOwnType, index);
          break;
        }
        case ComponentBinaryType::Borrow: {
          Index index;
          CHECK_RESULT(ReadU32Leb128(&index, "type index"));
          COMPONENT_CALLBACK(OnBorrowType, index);
          break;
        }
        case ComponentBinaryType::AsyncFunc:
        case ComponentBinaryType::Func: {
        ...
        case ComponentBinaryType::Instance:
        case ComponentBinaryType::Component: {
          uint32_t count;
          CHECK_RESULT(ReadU32Leb128(&count, "definition count"));

          if (def_type == ComponentBinaryType::Instance) {
    ...
    while (true) {
      ...
      assert(back.type == ComponentBinaryType::Instance ||
             back.type == ComponentBinaryType::Component);

      if (back.i == 0) {
        if (back.type == ComponentBinaryType::Instance) {
          COMPONENT_CALLBACK0(EndInstanceType);
        } else {
          COMPONENT_CALLBACK0(EndComponentType);
        }
...
Result BinaryReader::ReadComponentExternal(bool is_import,
  ...
  if (is_component_export) {
    ...
    CHECK_RESULT(
        ReadComponentSort(&export_info->sort, &export_info->core_sort));
  ...
  if (type == ComponentBinaryType::Some) {
    ...
    external_info->core_sort = ComponentBinaryCoreSort::NonCore;
    external_info->external = ComponentBinaryExternal::Unused;

    CHECK_RESULT(ReadU8(&type_code, "extern type"));
    external_info->sort = static_cast<ComponentBinarySort>(type_code);
    switch (external_info->sort) {
      case ComponentBinarySort::Core:
        CHECK_RESULT(ReadU8(&type_code, "extern core type"));
        external_info->core_sort =
            static_cast<ComponentBinaryCoreSort>(type_code);
        if (external_info->core_sort != ComponentBinaryCoreSort::Module) {
      ...
      case ComponentBinarySort::Value:
        ...
        external_info->external =
            static_cast<ComponentBinaryExternal>(type_code);
        if (external_info->external != ComponentBinaryExternal::ValueEq &&
            external_info->external != ComponentBinaryExternal::ValueType) {
          ERROR("Unknown core type: %" PRIx8, type_code);
          return Result::Error;
        }
        break;
      case ComponentBinarySort::Type:
        ...
        external_info->external =
            static_cast<ComponentBinaryExternal>(type_code);
        if (external_info->external != ComponentBinaryExternal::TypeEq &&
            external_info->external != ComponentBinaryExternal::TypeSubRes) {
          ERROR("Unknown core type: %" PRIx8, type_code);
          return Result::Error;
        }
        break;
      default:
        ERROR("Unknown type: %" PRIx8, type_code);
        return Result::Error;
    }
...
Result BinaryReader::ReadComponent() {
  ...
  while (true) {
    while (state_.offset < read_end) {
      ...
      switch (static_cast<ComponentBinarySection>(section_code)) {
        case ComponentBinarySection::CoreModule: {
          COMPONENT_CALLBACK(OnCoreModule, state_.data + state_.offset,
                             section_size, options_);
          state_.offset += section_size;
          break;
        }
        case ComponentBinarySection::Component: {
          uint32_t magic = 0;
        ...
        case ComponentBinarySection::Alias: {
          uint32_t count;
          CHECK_RESULT(ReadU32Leb128(&count, "count"));
        ...
        case ComponentBinarySection::Type: {
          uint32_t count;
          CHECK_RESULT(ReadU32Leb128(&count, "count"));
        ...
        case ComponentBinarySection::Import: {
          uint32_t count;
          CHECK_RESULT(ReadU32Leb128(&count, "count"));
        ...
        case ComponentBinarySection::Export: {
          uint32_t count;
          CHECK_RESULT(ReadU32Leb128(&count, "count"));
